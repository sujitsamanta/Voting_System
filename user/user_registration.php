...
if (isset($_POST['add_data'])) {
  ...
  if ($row_count == 1) {
    ...
    $sql = "INSERT INTO users(name,email,dob,gender,phone,village,post_office,police_station,district,state,pin_code,password) 
VALUES ('$name','$email','$dob','$gender','$phone','$village','$po','$ps','$district','$state','$pin','$pass')";
...
<body
  class="relative min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 flex items-center justify-center px-4">
      ...
      <form id="registrationForm" class="space-y-8" action="#" method="post" novalidate>
        ...
        <div>
            ...
            <span class="text-blue-600">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5">
                ...
                <path fill-rule="evenodd"
                  d="M.458 20.292A11.944 11.944 0 0112 18c3.183 0 6.086 1.24 8.242 3.292a.75.75 0 001.058-1.062A13.444 13.444 0 0012 16.5c-3.694 0-7.053 1.41-9.3 3.73a.75.75 0 101.058 1.062z"
                  clip-rule="evenodd" />
              </svg>
            </span>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <input autocomplete="bday" type="date" name="dob"
                class="w-full border border-slate-300 bg-slate-50 rounded-md px-3 py-2 focus:bg-white focus:ring-2 focus:ring-blue-600 focus:border-blue-600"
                value="<?php echo isset($_SESSION['form_data']['dob']) ? htmlspecialchars($_SESSION['form_data']['dob']) : ''; ?>">
            </div>
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">Phone Number</label>
              <input autocomplete="tel" inputmode="numeric" pattern="\d{10}" type="tel" name="phone"
                class="w-full border border-slate-300 bg-slate-50 rounded-md px-3 py-2 focus:bg-white focus:ring-2 focus:ring-blue-600 focus:border-blue-600"
            ...
            <div class="md:col-span-2">
              ...
              <div class="flex items-center gap-6 border border-slate-300 rounded-md px-3 py-2 bg-slate-50">
                <label class="inline-flex items-center gap-2 text-slate-700"><input type="radio" name="gender"
                    value="male" class="h-4 w-4" <?php if (isset($_SESSION['form_data']['gender']) && $_SESSION['form_data']['gender'] == 'male')
                      echo 'checked'; ?>> <span>Male</span></label>
                <label class="inline-flex items-center gap-2 text-slate-700"><input type="radio" name="gender"
                    value="female" class="h-4 w-4" <?php if (isset($_SESSION['form_data']['gender']) && $_SESSION['form_data']['gender'] == 'female')
                      echo 'checked'; ?>> <span>Female</span></label>
              </div>
            </div>
          </div>
        </div>
        ...
        <div>
          <div class="flex items-center gap-2 mb-4">
            <span class="text-blue-600">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5">
                <path fill-rule="evenodd"
                  d="M11.47 3.841a.75.75 0 011.06 0l7.07 7.07a7.5 7.5 0 11-10.607 10.607L3.923 15.55a7.5 7.5 0 0110.607-10.607l-3.06 3.06a3 3 0 10-4.243 4.243l4.596 4.597a3 3 0 104.243-4.243l-4.596-4.596a4.5 4.5 0 116.364 6.364l-4.596 4.596a4.5 4.5 0 11-6.364-6.364l7.07-7.07z"
                  clip-rule="evenodd" />
              </svg>
            </span>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div>
              ...
              <input autocomplete="address-line1" type="text" name="village"
                class="w-full border border-slate-300 bg-slate-50 rounded-md px-3 py-2 focus:bg-white focus:ring-2 focus:ring-blue-600 focus:border-blue-600"
                placeholder="Village"
                value="<?php echo isset($_SESSION['form_data']['village']) ? htmlspecialchars($_SESSION['form_data']['village']) : ''; ?>">
            </div>
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">Post Office</label>
              <input autocomplete="address-line2" type="text" name="po"
                class="w-full border border-slate-300 bg-slate-50 rounded-md px-3 py-2 focus:bg-white focus:ring-2 focus:ring-blue-600 focus:border-blue-600"
                placeholder="Post Office"
                value="<?php echo isset($_SESSION['form_data']['po']) ? htmlspecialchars($_SESSION['form_data']['po']) : ''; ?>">
            </div>
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">Police Station</label>
              <input autocomplete="address-line3" type="text" name="ps"
                class="w-full border border-slate-300 bg-slate-50 rounded-md px-3 py-2 focus:bg-white focus:ring-2 focus:ring-blue-600 focus:border-blue-600"
                placeholder="Police Station"
                value="<?php echo isset($_SESSION['form_data']['ps']) ? htmlspecialchars($_SESSION['form_data']['ps']) : ''; ?>">
            </div>
            <div>
            ...
            <div>
              ...
              <input autocomplete="address-level1" type="text" name="state"
                class="w-full border border-slate-300 bg-slate-50 rounded-md px-3 py-2 focus:bg-white focus:ring-2 focus:ring-blue-600 focus:border-blue-600"
                placeholder="State"
            ...
            <div>
              ...
              <input autocomplete="postal-code" type="text" inputmode="numeric" pattern="\\d{6}" name="pincode"
                class="w-full border border-slate-300 bg-slate-50 rounded-md px-3 py-2 focus:bg-white focus:ring-2 focus:ring-blue-600 focus:border-blue-600"
                placeholder="6-digit pincode"
                value="<?php echo isset($_SESSION['form_data']['pincode']) ? htmlspecialchars($_SESSION['form_data']['pincode']) : ''; ?>">
            </div>
            <!-- <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">Pincode</label>
        ...
        <div>
          <div class="flex items-center gap-2 mb-4">
            <span class="text-blue-600">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5">
                <path fill-rule="evenodd"
                  d="M12 1.5l8.485 4.243v5.73c0 5.246-3.61 10.12-8.485 11.027-4.875-.907-8.485-5.78-8.485-11.028V5.743L12 1.5zm0 6.75a3 3 0 00-3 3v1.5a.75.75 0 001.5 0V11.25a1.5 1.5 0 113 0v1.5a.75.75 0 001.5 0V11.25a3 3 0 00-3-3z"
                  clip-rule="evenodd" />
              </svg>
            </span>
            ...
            <div>
              ...
              <div class="relative">
                <input id="password" type="password" name="password"
                  class="w-full border border-slate-300 bg-slate-50 rounded-md px-3 py-2 pr-12 focus:bg-white focus:ring-2 focus:ring-blue-600 focus:border-blue-600"
                  placeholder="At least 6 characters"
                  value="<?php echo isset($_SESSION['form_data']['password']) ? htmlspecialchars($_SESSION['form_data']['password']) : ''; ?>">
                <button type="button" id="togglePassword"
                  class="absolute inset-y-0 right-0 px-3 text-sm text-blue-600 hover:text-blue-700">Show</button>
              </div>
              <div class="mt-3">
        ...
        <div class="flex items-center justify-between pt-2">
          ...
          <div class="flex items-center gap-3">
            <button type="reset"
              class="px-4 py-2 rounded-md border border-slate-300 text-slate-700 hover:bg-slate-50">Reset</button>
            <button type="submit" name="add_data"
              class="bg-blue-600 text-white px-6 py-2 rounded-md hover:bg-blue-700 transition disabled:opacity-60 disabled:cursor-not-allowed">Register</button>
          </div>
        </div>
      </form>
    </div>
